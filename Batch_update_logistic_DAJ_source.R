...
dlogr.step <-
  function (x.t, y.t, betahat.tm1, varbetahat.tm1, tune.mat) 
  {
    ...
    lambda <- tune.mat[which.max(temp), ] #choose the value of lambda which maximises equation (8) in McCormick et al (2012)
    ...
    Del2 <- -solve(Rhat.t) - (t(x.t) * matrix(rep(yhat.t * (1 - 
                                                              yhat.t), dim(x.t)[2]), nrow = dim(x.t)[2], byrow = TRUE)) %*% 
              x.t
...
function (tune.vec, x.t, y.t, betahat.tm1, varbetahat.tm1) 
{
  ...
  Del2 <- -solve(Rhat.t) - (t(x.t) * matrix(rep(yhat.t * (1 - 
    ...
    x.t
  betahat.t <- betahat.tm1 - (solve(Del2) %*% Del1)
  if (max(Rhat.t - t(Rhat.t)) > 0) {
    p.theta <- dmnorm(t(betahat.t), t(betahat.tm1), round(Rhat.t, #multivariate normal distribution
                                                          7))
  }
  else {
    p.theta <- dmnorm(t(betahat.t), t(betahat.tm1), Rhat.t)
  }
